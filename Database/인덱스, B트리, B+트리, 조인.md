...
- Branch node들과 Data node(레코드)로 구성이 되어있다.
  - `Branch node`: leaf node를 제외한 나머지 node들. key만 담아두고 data는 담지 않는다. 
  - `Data node`: leaf node. 오직 리프노드에만 key와 data를 저장하고, 리프 노드끼리 Linked List로 연결되어 있다.
